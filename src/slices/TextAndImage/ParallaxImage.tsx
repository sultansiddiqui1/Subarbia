...
export function ParallaxImage({
  ...
  //    using a ref because when we update a ref it doesnt trigger a rerender. the useState hook  would rerender, which would be 60 times for second for us.
  ...
  useEffect(() => {
    ...
    function animationFrame() {
      ...
      // per frame move the 1 percent of the gap kinda
      const newX = currentX + (targetX - currentX) * 0.1;
      const newY = currentY + (targetY - currentY) * 0.1;

      currentPositon.current = { x: newX, y: newY };
      ...
      // requestAnimationFrame is a browser fucntion and we are using it over say setInterval becuase this will sync with the browsers refresh rate
  ...
  return (
    <div className={clsx("grid grid-cols-1 place-items-center", className)}>
      {/* stacking the images on top of each other by using grid and placing both of the images in the same row and column.  */}
      <div
        ref={backgroundRef}
