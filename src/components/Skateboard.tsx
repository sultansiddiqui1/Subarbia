...
export function Skateboard({
  ...
  wheelTextures.forEach((texture) => {
    texture.flipY = false;
    texture.colorSpace = THREE.SRGBColorSpace; // JUST MAKING THE COLORS A BIT BRIGHTER
  });
  ...
  const gripTapeMaterial = useMemo(() => {
    const material = new THREE.MeshStandardMaterial({
      map: gripTapeDiffuse,
      bumpMap: gripTapeRoughness, // what does it look like when hit by  light
      roughnessMap: gripTapeRoughness, // how shiny
      bumpScale: 3.5,
      roughness: 0.8, //overall roughness
      color: "#555555",
    });
    ...
    if (gripTapeDiffuse) {
      ...
      //sharpness or bluriness from the glacning angles, better to add a power of 2
  ...
  const truckMaterial = useMemo(
    () =>
      new THREE.MeshStandardMaterial({
        ...
        normalScale: new THREE.Vector2(0.3, 0.3), // the vector2 is basically holding an x and y value. the vector3 also holds a z value.
        metalness: 0.8,
  ...
  // sometimes the text flips in threejs
  ...
  //wheels:
  // sometimes  in threejs we see that texts can be flipped.
  ...
  // constant wheel spin for the hero section:
  useFrame(() => {
