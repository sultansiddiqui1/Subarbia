...
export function FooterPhysics({
  ...
  // Handle mobile detection, like in the mobile screens:
  useEffect(() => {
    const handleResize = () => {
  ...
  // Fewer boards on mobile, not all 8 bit only 3 for mobile
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ...
      { threshold: 0.5 } // Trigger at 50% so users see the boards drop and not the boards on the ground
    );
  ...
  useEffect(() => {
    ...
    // Create walls/boundaries around the scene to keep boards in and make sure they cant be dragged outside them
